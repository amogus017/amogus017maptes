...
// Calculate optimal font size based on territory size and zoom
    calculateFontSize = (territorySize, zoom) => {
        // Base size calculation: larger territories get larger text
        const baseSize = Math.max(8, Math.min(24, territorySize * 15));
        
        // Zoom multiplier: text gets bigger when zoomed in
        const zoomMultiplier = Math.pow(1.3, zoom - 5); // Base zoom is 5
        
        const finalSize = baseSize * zoomMultiplier;
        
        // Clamp between reasonable bounds
        return Math.max(10, Math.min(32, finalSize));
    }

    // Calculate letter spacing based on font size
    calculateLetterSpacing = (fontSize) => {
        // Smaller text = tighter spacing, larger text = wider spacing
        return Math.max(0.5, fontSize * 0.08);
    }
   // Create custom div icon for territory labels with dynamic sizing
    createLabelIcon = (name, color, territorySize, zoom) => {
        const fontSize = this.calculateFontSize(territorySize, zoom);
        const letterSpacing = this.calculateLetterSpacing(fontSize);
        const outlineWidth = Math.max(1.5, fontSize * 0.1);
        const shadowBlur = Math.max(4, fontSize * 0.3);
