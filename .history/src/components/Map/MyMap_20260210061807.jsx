...
function TerritoryLabels({ empires, currentYear }) {
    ...
    const newLabelData = empires.map((empire) => {
        ...
        try {
            // Store BOTH latLng AND pixel position
            const layerPoint = map.latLngToLayerPoint(labelPlacement.position);
            
            return {
                id: empire.id,
                name: info.name,
                latLng: labelPlacement.position, // Keep the lat/lng reference
                x: layerPoint.x,
                y: layerPoint.y,
...
// NEW: Update pixel positions during zoom without recalculating geometry
const updateLabelPositions = React.useCallback(() => {
    if (!labelData.length || !map) return;
    
    setLabelData(prevLabels => 
        prevLabels.map(label => {
            const layerPoint = map.latLngToLayerPoint(label.latLng);
            return {
                ...label,
                x: layerPoint.x,
                y: layerPoint.y
...
}, [map, labelData.length]); // Note: using labelData.length instead of labelData to avoid infinite loop
...
React.useEffect(() => {
    ...
    // Recalculate label geometry only when zoom/pan ends
    map.on('moveend', updateLabels);
    map.on('zoomend', updateLabels);
    
    // Update pixel positions during zoom to keep labels stuck
...
}, [map, updateLabels, updateLabelPositions, isReady]);
