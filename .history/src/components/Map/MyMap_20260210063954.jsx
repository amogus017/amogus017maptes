...
function TerritoryLabels({ empires, currentYear }) {
    ...
    // Calculate labels once on mount and when map events fire
    const calculateLabels = React.useCallback(() => {
        if (!map) return;
        ...
        setLabelData(prev => {
    if (prev.length !== newLabelData.length) return newLabelData;

    for (let i = 0; i < prev.length; i++) {
        const a = prev[i];
        const b = newLabelData[i];
        if (
            a.id !== b.id ||
            a.x !== b.x ||
            a.y !== b.y ||
    ...
    // Set up map event listeners and initial calculation - ONCE ONLY
    React.useEffect(() => {
        if (!map) return;

        console.log('🗺️ Setting up map event listeners (ONCE)');
    ...
    }, [map, calculateLabels, updateOverlayTransform]);
...
class MyMap extends Component {
    ...
    render() {
        ...
        // Create a stable key that changes when empires actually change
        const labelsKey = activeEmpires.map(e => e.id).sort().join('-') + '-' + this.state.currentYear;
        ...
        return (
          <div className="map-wrapper">
            ...
            >
              ...
              {/* Territory Labels - remounts when key changes */}
              <TerritoryLabels 
                key={labelsKey}
