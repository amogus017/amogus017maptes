...
function TerritoryLabels({ empires, currentYear }) {
    ...
    const updateLabels = React.useCallback(() => {
        ...
        setLabels(newLabels); // Update state for render
    ...
    const updateLabelPositions = React.useCallback(() => {
        if (!overlayRef.current || !map || labelsRef.current.length === 0) return;
        ...
        // Update each label's pixel position using ref
    ...
    }, [map]); // Only depend on map, not labels!
        ...
        return (
          <div className="map-wrapper">
            ...
            >
              ...
              {/* Territory Labels - Smooth during zoom and pan, no infinite loop */}
