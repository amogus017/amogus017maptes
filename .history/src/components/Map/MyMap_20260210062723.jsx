...
function TerritoryLabels({ empires, currentYear }) {
    ...
    const updateLabelsRef = React.useRef(null);
    const updateOverlayTransformRef = React.useRef(null);
    
    // Create stable reference for empires using empire IDs
    const empireIds = React.useMemo(() => 
        empires.map(e => e.id).join(','), 
        [empires]
    );
    ...
    // Update the ref for overlay transform
    updateOverlayTransformRef.current = () => {
    ...
    // Recalculate labels when empires or year changes
    // Use empireIds instead of empires to prevent infinite loops
    React.useEffect(() => {
        if (updateLabelsRef.current) {
            updateLabelsRef.current();
        }
    }, [empireIds, currentYear]);

    // Set up map event listeners ONCE
    React.useEffect(() => {
        if (!map) return;

        // Create stable handler functions that call the refs
        const handleMoveEnd = () => updateLabelsRef.current?.();
        const handleZoomEnd = () => updateLabelsRef.current?.();
        const handleMove = () => updateOverlayTransformRef.current?.();
        const handleZoom = () => updateOverlayTransformRef.current?.();
    ...
    }, [map]); // Only depends on map - runs once when map is ready
