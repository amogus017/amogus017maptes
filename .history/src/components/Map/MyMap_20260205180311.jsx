...
import "./MyMap.css";
import { calculateLabelPlacement, createPolygonClipPath } from './labelUtils';
...
function TerritoryLabels({ empires, currentYear }) {
    ...
    const updateLabels = React.useCallback(() => {
        ...
        console.log('🏷️ Updating labels for', empires.length, 'empires at zoom', zoom);

        const newLabels = empires.map((empire) => {
            const info = getTerritoryInfo(empire.id, currentYear);
            
            // Pass map and zoom for pixel-based sizing
            const labelData = calculateLabelPlacement(empire.boundary, zoom, map);
            ...
            try {
                ...
                // Create clip path to confine label within boundaries
                const clipPath = createPolygonClipPath(empire.boundary, map);
                
                console.log(`Label for ${info.name}:`, {
                    position: point,
                    fontSize: labelData.fontSize,
                    pixelSize: `${labelData.pixelWidth}x${labelData.pixelHeight}`,
        ...
        console.log('🏷️ Rendered labels:', newLabels.map(l => ({name: l.name, fontSize: l.fontSize})));
        setLabels(newLabels);
    }, [empires, currentYear, map, isReady, zoom]);
        ...
        return (
          <div className="map-wrapper">
            ...
            >
              ...
              {/* Territory Labels with automatic sizing and boundary clipping */}
