 ...
 * Now with automatic sizing based on territory pixel size and zoom level
 */
export function calculateLabelPlacement(geojson, zoom = 5, map = null) {
    ...
    // IMPROVED: Calculate font size based on actual pixel dimensions of territory
    let fontSize = 20; // Default fallback
    let pixelWidth = 0;
    let pixelHeight = 0;
    ...
    if (map) {
        try {
            ...
            pixelWidth = Math.abs(bottomRight.x - topLeft.x);
            pixelHeight = Math.abs(bottomRight.y - topLeft.y);
            
            // Calculate font size based on smaller dimension (width or height)
            // This ensures labels fit within the territory bounds
            const minDimension = Math.min(pixelWidth, pixelHeight);
            const maxDimension = Math.max(pixelWidth, pixelHeight);
            
            // Font size is proportional to the smaller dimension
            // Divided by 3-4 to leave padding and ensure it fits
            fontSize = minDimension / 4;
            
            // Additional scaling based on aspect ratio
            // Very elongated territories can have slightly larger text
            if (aspectRatio > 2) {
                fontSize = minDimension / 3.5;
            }
            
            // Scale with zoom level for better visibility
            const zoomFactor = Math.pow(1.15, zoom - 5);
            fontSize = fontSize * zoomFactor;
            
        } catch (e) {
            // Fallback to geographic area calculation
            const area = bounds.width * bounds.height;
            fontSize = Math.sqrt(area) * 100;
        }
    } else {
        // No map available - use geographic bounds
        const area = bounds.width * bounds.height;
        fontSize = Math.sqrt(area) * 100;
    }
    
    // Clamp to reasonable bounds
    // Min: 10px (for very small territories)
    // Max: 48px (for very large territories)
...
/**
 * Create CSS clip-path polygon from GeoJSON coordinates
 * This confines the label within the territory boundaries
 */
export function createPolygonClipPath(geojson, map) {
    if (!map) return 'none';
    ...
    if (geojson.type === 'FeatureCollection' && geojson.features) {
        geojson.features.forEach(feature => {
            ...
            if (geometry.type === "Polygon") {
                ...
                // Use first/largest polygon for clipping
                geometry.coordinates.forEach((polygon, idx) => {
                    if (idx === 0) { // Use first polygon
