...
const Victoria3Timeline = ({ onYearChange }) => {
  ...
  const frameRef = useRef(null); // tracks the slider-frame padding offset
  // Detect mobile
  useEffect(() => {
  ...
  const scrollToYear = useCallback((newYear, smooth = false) => {
    ...
    const MOBILE_PADDING = isMobile ? frame.clientWidth / 2 : 0;
  ...
  // On mobile mount/switch: scroll track to current year so marker aligns correctly
  useEffect(() => {
    if (isMobile) {
    ...
    <div className="victoria3-timeline">
      ...
      <div className="v3-slider-container">
        ...
        <div
          ...
          ref={(el) => { scrollRef.current = el; frameRef.current = el; }}
          onScroll={(e) => {
            if (isMobile) {
               // Subtract padding (20px) so center maps correctly to pixel coords
      const PADDING = 20;
      const viewportCenter = frame.scrollLeft + frame.clientWidth / 2 - PADDING;
      ...
      onYearChange(snapped);
        ...
        >
          ...
          {/* Mobile needle — positioned absolute, always at visual center */}
{isMobile && (
