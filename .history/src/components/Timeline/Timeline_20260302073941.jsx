...
const Victoria3Timeline = ({ onYearChange }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    // ✅ FIX: Inverted - dragging right should move timeline left (go forward in time)
    // Think of it like grabbing and sliding a physical ruler
    const yearsDelta = -(deltaX / containerWidth) * VIEWPORT_YEARS;
  ...
  // ✅ CORRECTED: Calculate CSS transform to properly center the viewport
  const getRulerTransform = () => {
    const totalYears = MAX_YEAR - MIN_YEAR; // 1200 years
    
    // Where is viewportCenter on the ruler? (0 to 1)
    const centerPositionOnRuler = (viewportCenter - MIN_YEAR) / totalYears;
    
    // The ruler is 300% wide. centerPositionOnRuler tells us where on that 300% our year is.
    // We want that position to appear at 50% of the viewport.
    // 
    // If ruler is at 0% transform: left edge of ruler = left edge of viewport
    // We need: (centerPositionOnRuler * rulerWidth) = 50% of viewport
    // 
    // Since ruler is 300% and viewport is 100%, we need:
    // transform = -(centerPositionOnRuler * 300 - 50)
    
    const rulerWidthPercent = (totalYears / VIEWPORT_YEARS) * 100; // 300
    const shift = -(centerPositionOnRuler * rulerWidthPercent - 50);
