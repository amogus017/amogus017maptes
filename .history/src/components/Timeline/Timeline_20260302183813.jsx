...
const Victoria3Timeline = ({ onYearChange }) => {
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (scrollRef.current) {
                        ...
                        const EDGE_THRESHOLD = viewportWidth * 0.15;
  ...
  const targetScroll = Math.max(0, thumbPx - viewportWidth / 2);
  ...
  const animate = () => {
    ...
    if (Math.abs(diff) < 1) {
      frame.scrollLeft = targetScroll;
      return;
    }
    
    frame.scrollLeft += diff * 0.12; // ⬅ speed factor: lower = slower (try 0.05–0.2)
